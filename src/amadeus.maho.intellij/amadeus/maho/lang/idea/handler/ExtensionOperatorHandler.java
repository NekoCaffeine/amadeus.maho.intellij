...
public class ExtensionOperatorHandler extends BaseHandler<Extension.Operator> {
    ...
    @Override
    public void collectRelatedTarget(final PsiModifierListOwner tree, final Extension.Operator annotation, final PsiAnnotation annotationTree, final Set<PsiNameIdentifierOwner> targets) {
        if (tree instanceof PsiMethod method)
            Stream.of(method.getContainingClass().getAllMethods())
                    .cast(DerivedMethod.class)
                    .filter(derived -> derived.source() == method)
                    .forEach(targets::add);
    }
